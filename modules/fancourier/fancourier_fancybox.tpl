...
{literal}
	$('#soLink').fancybox({
			...
			'type'					: 'iframe',
			onStart		:	function() {
				$('#soLink').attr('href', 'modules/fancourier/redirect.php' + serialiseInput(soInputs))
			},
			onClosed    :   function() {
         	   $.ajax({
			       type: 'GET',
			       url: baseDir+'/modules/fancourier/ajax.php',
			       async: false,
			       cache: false,
			       dataType : "json",
			       data: "token=" + soToken,
	...
	function so_click()
	{
		if (opc)
		{
			if (!already_select_delivery || !$('#edit_fancourier').length)
...
function modifyCarrierLine()
{
	...
	$('#button_fancourier').remove();

	if ((carrier.val() == soCarrierId) || (carrier.val() == soCarrierId+',')) {
		$(container).parent().prepend('<a style="margin-left:5px;" class="exclusive" id="button_fancourier" href="#" onclick="redirect();return;" >{/literal}{$select_label}{literal}</a>');
	}
