...
include('../../init.php');
require_once(_PS_MODULE_DIR_.'fancourier/classes/SCFields.php');

// Init the Context (inherit FANCourier and handle error)
...
if (!$so->checkErrors($errors_codes, SCError::REQUIRED))
{
	...
	foreach ($so->getFields(SCFields::REQUIRED) as $field)
		if (!isset($return[$field]))
			$errors_list[] = $so->l('This key is require for FANCourier:').$field;
}
else
...
function saveOrderShippingDetails($idCart, $idCustomer, $soParams, $so_object)
{
	...
	$db = Db::getInstance();
	$db->executeS('SELECT * FROM '._DB_PREFIX_.'fancourier_delivery_info WHERE id_cart = '.(int)($idCart).' AND id_customer ='.(int)($idCustomer));
	$numRows = (int)($db->NumRows());
	if ($numRows == 0)
	{
		$sql = 'INSERT INTO '._DB_PREFIX_.'fancourier_delivery_info
	...
	{
		$table = _DB_PREFIX_.'fancourier_delivery_info';
		$values = array();
		...
		if ($so_object->delivery_mode == SCFields::RELAY_POINT)
		{
			...
			(isset($deliveryMode[$soParams['DELIVERYMODE']]) ? $values['prfirstname'] = $deliveryMode[$soParams['DELIVERYMODE']] : $values['prfirstname'] = 'FAN Courier');
