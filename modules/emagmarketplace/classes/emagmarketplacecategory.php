...
class EmagMarketplaceCategory extends ObjectModel
{
	public $id;
	...
	public $emag_category_id;
	public $emag_family_type_id;
	public $commission;
	public $sync_active;
	
	public $emag_category_name;
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'emag_category_id' => 	array('type' => self::TYPE_INT),
			'emag_family_type_id' =>array('type' => self::TYPE_INT),
			'commission' => 		array('type' => self::TYPE_FLOAT),
			'sync_active' => 		array('type' => self::TYPE_BOOL),
		),
	);
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		if (Tools::getIsset('checkBoxShopAsso_category'))
			foreach (Tools::getValue('checkBoxShopAsso_category') as $row)
			{
				foreach ($row as $id_shop => $value)
				{
					$position = EmagMarketplaceCategory::getLastPosition((int)$this->id_parent, $id_shop);
			...
			{
				$position = EmagMarketplaceCategory::getLastPosition((int)$this->id_parent, $shop['id_shop']);
				$this->addPosition($position, $shop['id_shop']);
			}
		if (!isset($this->doNotRegenerateNTree) || !$this->doNotRegenerateNTree)
			EmagMarketplaceCategory::regenerateEntireNtree();
	...
	public function update($null_values = false)
	{
		...
		if ($this->getDuplicatePosition())
		{
			...
			foreach ($assos as $shop)
				$this->addPosition(EmagMarketplaceCategory::getLastPosition((int)$this->id_parent, $shop['id_shop']), $shop['id_shop']);
		}
	...
	public function updateEmagCategoryID()
	{
		$result = 0;
		Db::getInstance()->execute('
			INSERT INTO `'.pSQL(_DB_PREFIX_.$this->def['extra_table']).'`
			SET
			`'.pSQL($this->def['primary']).'` = '.(int)$this->id.',
			`emag_category_id` = '.(int)$this->emag_category_id.'
		...
		if (Db::getInstance()->Affected_Rows())
		{
			$result = 2;
			Db::getInstance()->execute('
				UPDATE `'.pSQL(_DB_PREFIX_.$this->def['extra_table']).'`
				SET
				`emag_family_type_id` = 0
				WHERE `'.pSQL($this->def['primary']).'` = '.(int)$this->id.'
			');
		}
		elseif (Db::getInstance()->Insert_ID())
		{
			$result = 2;
		}
		else
	...
	public function updateEmagFamilyTypeID()
	{
		return Db::getInstance()->execute('
			INSERT INTO `'.pSQL(_DB_PREFIX_.$this->def['extra_table']).'`
			SET
			`'.pSQL($this->def['primary']).'` = '.(int)$this->id.',
			`emag_family_type_id` = '.(int)$this->emag_family_type_id.'
			ON DUPLICATE KEY UPDATE
			`emag_family_type_id` = '.(int)$this->emag_family_type_id.'
		');
	}
	...
	public function toggleStatus()
	{
		// Update active status on object
	 	$this->sync_active = !(int)$this->sync_active;
	...
	public function recurseLiteCategTree($max_depth = 3, $current_depth = 0, $id_lang = null, $excluded_ids_array = null)
	{
		...
		if (($max_depth == 0 || $current_depth < $max_depth) && $subcats && count($subcats))
			foreach ($subcats as &$subcat)
			{
				...
				{
					$categ = new EmagMarketplaceCategory($subcat['id_category'], $id_lang);
		...
		return array(
			...
			'name' => $this->name,
			'desc'=> EmagMarketplaceCategory::getDescriptionClean($this->description),
			'children' => $children
		);
	}
	...
	public static function recurseCategory($categories, $current, $id_category = 1, $id_selected = 1)
	{
		...
		str_repeat('&nbsp;', $current['infos']['level_depth'] * 5).Tools::stripslashes($current['infos']['name']).'</option>';
		if (isset($categories[$id_category]))
			foreach (array_keys($categories[$id_category]) as $key)
				EmagMarketplaceCategory::recurseCategory($categories, $categories[$id_category][$key], $key, $id_selected);
	}
	...
	public function delete()
	{
		...
		EmagMarketplaceCategory::cleanPositions($this->id_parent);

		/* Delete category images and its children images */
		$tmp_category = new EmagMarketplaceCategory();
	...
	public function deleteSelection($categories)
	{
		...
		{
			$category = new EmagMarketplaceCategory($id_category);
	...
	public function calcLevelDepth()
	{
		...
		$parent_category = new EmagMarketplaceCategory($this->id_parent);
	...
	public static function regenerateEntireNtree()
	{
		...
		if (isset($categories_array[0]) && $categories_array[0]['subcategories'])
			EmagMarketplaceCategory::_subTree($categories_array, $categories_array[0]['subcategories'][0], $n);
	}
	...
	protected static function _subTree(&$categories, $id_category, &$n)
	{
		...
		if (isset($categories[(int)$id_category]['subcategories']))
			foreach ($categories[(int)$id_category]['subcategories'] as $id_subcategory)
				EmagMarketplaceCategory::_subTree($categories, (int)$id_subcategory, $n);
		$right = (int)$n++;
	...
	public static function getRootCategory($id_lang = null, Shop $shop = null)
	{
		...
		if (!$shop)
			...
			return new EmagMarketplaceCategory($shop->getCategory(), $id_lang);
		$is_more_than_one_root_category = count(EmagMarketplaceCategory::getCategoriesWithoutParent()) > 1;
		if ((!Shop::isFeatureActive() && $is_more_than_one_root_category) ||
			...
			$category = EmagMarketplaceCategory::getTopCategory($id_lang);
		else
			$category = new EmagMarketplaceCategory($shop->getCategory(), $id_lang);
	...
	public function getParentsCategories($id_lang = null)
	{
		...
		$categories = null;
		$id_current = $this->id;
		if (count(EmagMarketplaceCategory::getCategoriesWithoutParent()) > 1 && Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && count(Shop::getShops(true, null, true)) != 1)
			$context->shop->id_category = EmagMarketplaceCategory::getTopCategory()->id;
	...
	*/
	...
	public function inShop(Shop $shop = null)
	{
		...
		if (!$interval = EmagMarketplaceCategory::getInterval($shop->getCategory()))
			return false;
	...
	public static function getTopCategory($id_lang = null)
	{
		...
		FROM `'._DB_PREFIX_.'category`
		WHERE `id_parent` = 0');
		return new EmagMarketplaceCategory($id_category, $id_lang);
	}
	...
	*/
	public static function updateFromShop($categories, $id_shop)
	{
		...
		EmagMarketplaceCategory::deleteCategoriesFromShop($id_shop);

		// and add $categories to this shop
		return EmagMarketplaceCategory::addToShop($categories, $id_shop);
	}
	...
	public static function addToShop(array $categories, $id_shop)
	{
		...
		foreach ($categories as $id_category)
		{
			$tab_categories[] = new EmagMarketplaceCategory($id_category);
		...
		foreach ($tab_categories as $category)
			$category->addPosition(EmagMarketplaceCategory::getLastPosition($category->id_parent, $id_shop), $id_shop);
