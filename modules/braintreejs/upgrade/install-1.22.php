...
function upgrade_module_1_22($object, $install = false)
{
	...
	if ((!$bt_version) || (empty($bt_version)) || ($bt_version < $object->version))
		Configuration::updateValue('BRAINTREE_VERSION', '1.22');

	//new setting to account for settlement pending transaction status.  We default to 'Payment Error'
	Configuration::updateValue('BRAINTREE_SETTLEMENT_PENDING_OS', Configuration::get('PS_OS_ERROR'));
