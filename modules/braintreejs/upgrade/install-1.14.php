...
function upgrade_module_1_14($object, $install = false)
{
	...
	if ((!$bt_version) || (empty($bt_version)) || ($bt_version < $object->version))
		Configuration::updateValue('BRAINTREE_VERSION', '1.14');

	$object->registerHook('postUpdateOrderStatus');	//1.4

	//delete the following old files
	$path = realpath(_PS_MODULE_DIR_.$object->name);
	$file1 = realpath($path.'/lib/Braintree/PartnerUser.php');
	$file2 = realpath($path.'/lib/Braintree/SubscriptionStatus.php');
	$file3 = realpath($path.'/lib/ssl/sandbox_braintreegateway_com.ca.crt');
	$file4 = realpath($path.'/lib/ssl/www_braintreegateway_com.ca.crt');
