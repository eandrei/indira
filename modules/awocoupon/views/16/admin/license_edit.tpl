...
<div class="panel awocoupon">
	<h3><i class="icon-tag"></i> {l s='License' mod='awocoupon'}</h3>
	...
	<br /><br />
		...
		<div class="width-100">
			<fieldset>{if $row->ispermanent=='yes'}<legend>{l s='Permanent License' mod='awocoupon'}</legend>{/if}
				...
				<tr><td class="key"><label class="control-label">{l s='Website' mod='awocoupon'}</label></td>
					<td><input type="text" size="75" name="website" value="{$row->website}" {if $row->ispermanent=='yes'}READONLY DISABLED class="readonly"{/if}></td>
				</tr>
				<tr><td class="key"><label class="control-label">{l s='License' mod='awocoupon'}</label></td>
					<td><input type="text" size="75" name="license" value="{$row->license}" {if $row->ispermanent=='yes'}READONLY DISABLED class="readonly"{/if}></td>
				</tr>
				{if $row->license!=''}
				<tr valign="top"><td class="key"><label class="control-label">{l s='Local Key' mod='awocoupon'}</label></td>
