...
<h2>{if $row->id>0}{l s='Edit' mod='awocoupon'}{else}{l s='New' mod='awocoupon'}{/if} {l s='Coupon' mod='awocoupon'}</h2>




<table class="admintable"><tr><td><h3>{l s='Function Type' mod='awocoupon'}</h3></td><td>&nbsp;&nbsp;{$lists.dd_function_type}</td></tr></table>
...
<form id="{$table}_form" name="adminForm" method="post" action="{$smarty.server.REQUEST_URI}"  onsubmit="return submitbutton();">
	...
	<div id="awocoupon_block_optional" class="cart_rule_tab">
		...
		<table class="admintable">
			...
			<tr id="tr_addtocart" style="display:none;"><td class="key" nowrap><label>{l s='Automatically add to cart "Get Y" product' mod='awocoupon'}</label></td>
