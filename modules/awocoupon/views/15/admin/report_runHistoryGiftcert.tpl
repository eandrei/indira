...
<center>
<div><font size="5">{l s='History of Uses - Gift Certificates' mod='awocoupon'}</font></div>
...
<form id="{$table}_form" name="adminForm" method="post" action="{$smarty.server.REQUEST_URI}">
	...
	<tr><td><INPUT TYPE="image" onclick="submitform('exportreports');" NAME="submit" src="{$awo_uri}/media/img/excel.gif" border="0" alt="Export CSV">
			<font size="4">{l s='History of Uses - Gift Certificates' mod='awocoupon'}</font>
	</td></tr>
	...
	<INPUT type="hidden" name="filename" value="history_uses_giftcerts_{if $start_date!=''} {$start_date}_{$end_date}{/if}.csv">
