...
function init() {
	if(document.adminForm.shop_id != undefined) {
		$j( "#shop_search" )
			.autocomplete({
				source: function( request, response ) {
					$j.getJSON( 
						base_url, 
						{option:my_option, task:'ajax_elements', type:'shop', tmpl:'component', no_html:1,term: request.term}, 
						response 
					);
				},
				minLength: 2,
				selectFirst: true,
				delay:0,
				select: function( event, ui ) { if(ui.item) document.adminForm.shop_id.value = ui.item.id; }
			})
			.attr("parameter_id", document.adminForm.shop_id.value)
			.bind("empty_value", function(event){ document.adminForm.shop_id.value = ''; })
...
function close_panes() {
		...
		$j("#extra_options1 .panel div.pane-slider").hide();
		$j("#extra_options2 .panel div.pane-slider").hide();
}
...
function funtion_type_change(is_edit) {
	...
	if(form.function_type.value == '') $j('#div_master_list').hide(); else $j('#div_master_list').show();
	
	if(form.function_type.value == '');
	else if(form.function_type.value == 'giftcert') {
		$j('#tr_coupon_code,#tr_published,#tr_coupon_value,#fs_optionals,#tr_startdate,#tr_expiration,#tr_exclude_giftcert,#div_asset1_list,#li_label_asset').show();
		
		$j('#div_asset1_label').html(str_asset);
		...
		$j('#div_asset2_label').html(str_shipping);
...
function funtion_type2_change(is_edit) {
	...
	if(form.function_type.value == 'parent') {
		...
		$j('#div_asset1_label').html(str_coupons);
		$j('#fs_assets,#tr_coupon_code,#tr_published,#tr_parent_type,#tr_num_of_uses_total,#tr_num_of_uses_percustomer,#fs_optionals,#div_asset1_list,#li_label_asset,#li_label_country').show();
		$j('#pn_asset').parent().show();
		...
		$j('#tr_coupon_code,#tr_published,#tr_discount_type,#tr_coupon_value_type,'+
		   ...
		   '#tr_expiration,#fs_users,#fs_assets,#tr_f2_mode,#td_userfields,#div_asset2_type,#div_asset1_type,#div_customer_list,#div_asset1_list,#li_label_customer,#li_label_asset,#li_label_country').show();
		...
		if(form.function_type.value == 'buy_x_get_y') {
			$j('#div_asset1_label').html(str_buy_x);
			$j('#div_asset2_label').html(str_get_y);
			$j('#pn_asset2').parent().show();
			asset_type_change(1,is_edit);
			asset_type_change(2,is_edit);
				
			$j('#tr_discount_type,#tr_or,#tr_coupon_value_def,#tr_exclude_giftcert,#tr_parent_type').hide();
			$j('#div_asset_qty,#div_asset2_qty,#tr_buy_xy_type,#tr_max_discount_qty,#tr_product_match,#tr_addtocart,#div_asset1_list,#div_asset2_list,#tr_exclude_special').show();
...
function asset_type_change(intype,is_edit) {
	...
	if(intype==1) {
		...
		/*else {
			if(form.asset2_function_type.value=='') form.asset2_function_type.value = 'product';		
			val = form.asset2_function_type.value;
		}*/
...
function hideall() {
	$j('#tr_coupon_code,#tr_published,#tr_parent_type,#tr_coupon_value_type,'+
	   ...
	   '#tr_expiration,#fs_users,#fs_assets,#tr_buy_xy_type,#tr_max_discount_qty,#div_customer_list,#div_asset1_list,#div_asset2_list,'+
	   '#li_label_customer,#li_label_asset,#li_label_country').hide()
	$j('#pn_user').parent().hide();
...
function dd_itemselectf(type) {
	...
	if(type=='asset') {
		...
		id = form.shop_id.value;
		name = form.shop_name.value;
		value_list_id = 'shoplist[]';
		value_list_name = 'shopnamelist['+id+']';
		tbl = 'tbl_shops';
	} else return;
...
function dd_itemselectg(type) {
	...
	if(type == 'asset') {
		...
		value_list_id = 'shoplist[]';
		tbl = 'tbl_shops';
		searchDD = form.elements['_shoplist'];
	} else return;
...
function dd_searchg(type) {
	if(type=='asset') {
		...
		var input_text = 'shop_search_txt';
		var searchDD = document.adminForm.elements['_shoplist'];
	} else return;
