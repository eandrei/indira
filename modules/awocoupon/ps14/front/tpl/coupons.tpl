...
{capture name=path}<a href="{$link->getPageLink('my-account.php', true)}">{l s='My account' mod='awocoupon'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='My Coupons' mod='awocoupon'}{/capture}
		...
		<table class="std tablefront">
			...
			{foreach from=$rows item='row'}
				<tr>
					...
					<td>{$row->startdate_str}</td>
					<td>{$row->expiration_str}</td>
				</tr>
			{/foreach}
			</tbody>
		</table>
...
<ul class="footer_links">
	<li><a href="{$link->getPageLink('my-account.php', true)}"><img src="{$img_dir}icon/my-account.gif" alt="" class="icon" /> {l s='Back to Your Account' mod='awocoupon'}</a></li>
