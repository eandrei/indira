...
function error_messages_debug() {
	jQuery("input.error_message").each(function() {
		jQuery(this).val(jQuery(this).parent().parent().parent().parent().find('label span').html());
	});
}
...
form.form_tab { border:1px solid #ccced7; min-height:404px; padding: 5px 10px 10px; }
form.form_tab  h4 { font-size:18px; font-weight:normal; margin-top:0;}


.tab-row .tab.selected {border:1px solid #ccc;border-bottom:none; background:#FFF6D3;margin:1px 0px -3px 0px;top:-2px}
.tab-row .tab {display:block; padding:7px; font-weight:bold; background:#EFEFEF}
ul.tab { list-style:none; margin:0;padding:0;}
ul.tab li {float:left;}
.cart_rule_tab { padding-top:12px; }
</style>
{/literal}
...
<div id="div_master_list">
	...
	<div>
		<div class="productTabs">
			<ul class="tab">
				<li class="tab-row"><a class="tab" id="awocoupon_block_link_general" href="javascript:displayCartRuleTab('general');">{l s='General' mod='aworewards'}</a></li>
				<li class="tab-row"><a class="tab" id="awocoupon_block_link_multiple" href="javascript:displayCartRuleTab('multiple');">{l s='Multiple Coupons' mod='awocoupon'}</a></li>
				<li class="tab-row"><a class="tab" id="awocoupon_block_link_giftcert" href="javascript:displayCartRuleTab('giftcert');">{l s='Gift Certificate' mod='aworewards'}</a></li>
				<li class="tab-row"><a class="tab" id="awocoupon_block_link_errorcode" href="javascript:displayCartRuleTab('errorcode');">{l s='Error Codes' mod='aworewards'}</a></li>
			</ul>
		</div>
	</div>
	...
	<div class="width-100">
		...
		<div id="awocoupon_block_general" class="cart_rule_tab">
			<h4>{l s='General' mod='aworewards'}</h4>
		...
		<div id="awocoupon_block_giftcert" class="cart_rule_tab">
			<h4>{l s='Gift Certificate' mod='aworewards'}</h4>
			<div class="separation"></div>
			...
			<tr valign="top"><td class="key"><label>{l s='Vendor Email Body' mod='awocoupon'}</label>
				...
				<td><textarea class="rte" cols="100" rows="20" id="params_giftcert_vendor_email" name="params[giftcert_vendor_email]">{$row->giftcert_vendor_email}</textarea></td>
			</tr>
		...
		<div id="awocoupon_block_errorcode" class="cart_rule_tab">
			<h4>{l s='Coupon Code Error Description' mod='aworewards'}</h4>
			<div class="separation"></div>
					...
					<div class="translatable">
						{foreach from=$languages item=language}
							<div class="lang_{$language.id_lang|intval}" style="display:{if $language.id_lang == $id_lang_default}block{else}none{/if};float:left">
								<input type="text" class="error_message" size="60" name="lang[{$language.id_lang|intval}][errNoRecord]" value="{if isset($row->languages[$language.id_lang|intval]->errNoRecord)}{$row->languages[$language.id_lang|intval]->errNoRecord}{/if}" maxlength="255">
							</div>
						{/foreach}
					</div>
				</td>
				...
				<tr><td class="key"><label><span>{l s='Shop does not have permission to use' mod='awocoupon'}</span></label></td>
					<td>
						<div class="translatable">
							{foreach from=$languages item=language}
								<div class="lang_{$language.id_lang|intval}" style="display:{if $language.id_lang == $id_lang_default}block{else}none{/if};float:left">
									<input type="text" class="error_message" size="60" name="lang[{$language.id_lang|intval}][errShopPermission]" value="{if isset($row->languages[$language.id_lang|intval]->errShopPermission)}{$row->languages[$language.id_lang|intval]->errShopPermission}{/if}" maxlength="255">
								</div>
							{/foreach}
						</div>
					</td>
			...
			<tr><td class="key"><label><span>{l s='Minimum value not reached' mod='awocoupon'}</span></label></td>
				<td>
					<div class="translatable">
						{foreach from=$languages item=language}
							<div class="lang_{$language.id_lang|intval}" style="display:{if $language.id_lang == $id_lang_default}block{else}none{/if};float:left">
								<input type="text" class="error_message" size="60" name="lang[{$language.id_lang|intval}][errMinVal]" value="{if isset($row->languages[$language.id_lang|intval]->errMinVal)}{$row->languages[$language.id_lang|intval]->errMinVal}{/if}" maxlength="255">
							</div>
						{/foreach}
					</div>
				</td>
			...
			<tr><td class="key"><label><span>{l s='Minimum product quantity not reached' mod='awocoupon'}</span></label></td>
				<td>
					<div class="translatable">
						{foreach from=$languages item=language}
							<div class="lang_{$language.id_lang|intval}" style="display:{if $language.id_lang == $id_lang_default}block{else}none{/if};float:left">
								<input type="text" class="error_message" size="60" name="lang[{$language.id_lang|intval}][errMinQty]" value="{if isset($row->languages[$language.id_lang|intval]->errMinQty)}{$row->languages[$language.id_lang|intval]->errMinQty}{/if}" maxlength="255">
							</div>
						{/foreach}
					</div>
				</td>
	...
	<br /><div class="margin-form"><input type="submit" value="   {l s='Save' mod='awocoupon'}   " name="submit" class="button"></div>
