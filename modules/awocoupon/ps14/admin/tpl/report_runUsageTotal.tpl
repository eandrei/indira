...
<center>
<div><font size="5">{l s='Coupon Usage vs. Total sales' mod='awocoupon'}</font></div>
...
<form action="{$awo_uri}/ajax.php" method="post" name="adminForm">
	...
	<table><tr>
		<td><table width="430" class="graph" cellspacing="6" cellpadding="0">
			<thead><tr><th colspan="3">{l s='Total Volume' mod='awocoupon'}</th></tr></thead>
			<tbody>{$barvolume}</tbody>
			<tfoot><tr><td colspan="3"></td></tr></tfoot>
			</table>
	</td><td>
			<table width="430" class="graph" cellspacing="6" cellpadding="0">
			<thead><tr><th colspan="3">{l s='Total Revenue' mod='awocoupon'}</th></tr></thead>
			<tbody>{$barrevenue}</tbody>
			<tfoot><tr><td colspan="3"></td></tr></tfoot>
			</table>
	</td></tr></table>
	<br><br>
	...
	<tr><td><INPUT TYPE="image" onclick="submitform('exportreports');" NAME="submit" src="{$awo_uri}/media/img/excel.gif" border="0" alt="Export CSV">
			<font size="4">{l s='Coupon Usage vs. Total sales' mod='awocoupon'}</font>
	</td></tr>
	...
	<INPUT type="hidden" name="filename" value="coupon_usage_total_{if $start_date!=''} {$start_date}_{$end_date}{/if}.csv">
	{$parameters}
</form>
{$arrstr.js}
