...
class pagination
{
	...
	{
		$this->currentPage = ((isset($_POST[$this->pageIndex]) && (int)($_POST[$this->pageIndex]) > 0)?(int)($_POST[$this->pageIndex]):1);
	}
	...
	function getLastUrl()
	{
		...
		if ($this->currentPage < $totalPages)
		{
			...
			//$url  = 'javascript:document.adminForm.page.value='.$totalPages.'; submitform();return false;'; # seyi_code
			$url  = 'javascript:document.adminForm.page.value='.$totalPages.'; submitform();'; # seyi_code
	...
	function getFirstUrl()
	{
		...
		if ($this->currentPage > 1)
		{
			...
			//$url  = 'javascript:document.adminForm.page.value=1; submitform();return false;'; # seyi_code
			$url  = 'javascript:document.adminForm.page.value=1; submitform();'; # seyi_code
	...
	function getPreviousUrl()
	{
		...
		if ($this->currentPage > 1 && $this->totalItems > 0 )
		{
			...
			//$url  = 'javascript:document.adminForm.page.value='.($this->currentPage -1).'; submitform();return false;'; # seyi_code
			$url  = 'javascript:document.adminForm.page.value='.($this->currentPage -1).'; submitform();'; # seyi_code
		...
		if ($this->totalItems > 0)
		{
			...
			for ($item = $itemStart; $item <= $itemEnd; $item++)
			{
				...
				{//it is not the active link
					...
					//$temUrl  = 'javascript:document.adminForm.page.value='.$item.'; submitform();return false;'; # seyi_code
					$temUrl  = 'javascript:document.adminForm.page.value='.$item.'; submitform();'; # seyi_code
	...
	function getPaginationHTML($type=1, $cssClass="pagination")
	{
		...
		switch($type)
		{
			...
			$itemPerPage = '';
			$itemPerPage .= "<select name=\"" . $this->limitIndex . "\" id=\"limit\" class=\"input inputLimit\" onchange=\"submitform();\">\n";
			...
			$output .= ' &nbsp; '.sprintf('Display # %s', $itemPerPage);
			$output .= "</span>";
			$output .= "<span class=\"pagination_parent\"><a href=\"#\" onclick=\"goParentFolder();\" id=\"pagination_parent_link\" title=\"\">&nbsp;</a></span>";
		}
