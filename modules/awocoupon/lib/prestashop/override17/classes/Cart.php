...
class Cart extends CartCore {
	...
	public function addCartRule($id_discount) {
		if(substr($id_discount,0,10)=='awocoupon-') {
			...
			if(!empty($coupon_code)) {
				...
				if(empty($rtn)) 
					return Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'awocoupon_cart` (id_cart, new_ids) VALUES ('.(int)$this->id.',"'.pSQL($coupon_code).'")'); // 1.7.0
	...
	public function getCartRules($filter = CartRule::FILTER_ACTION_ALL) {
		...
		foreach($coupon_session['processed_coupons'] as $id_discount=>$it) {
			...
			$arrdiscount['reduction_amount'] = $arrdiscount['obj']->reduction_amount = $arrdiscount['value_real'];
