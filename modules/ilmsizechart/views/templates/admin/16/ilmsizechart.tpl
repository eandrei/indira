...
<h3><i class="icon-list-ul"></i> {l s='Size Chart' mod='ilmsizechart'}</h3>
    ...
    <div class="form-group">
        ...
        <div class="col-lg-6 col-sm-6">
            <select name="ilmchartgroup">
                ...
                <option value="{$group.id_group|escape:'intval'}" {if ($value.id_group == $group.id_group)}selected="selected"{/if}>{$group.title|escape:'intval'}</option>
                {/foreach}
            </select>
        </div>
        <div class="clear">&nbsp;</div>
    ...
    <a class="btn btn-default pull-left" href="{$link->getAdminLink('AdminModules')|escape:'htmlall':'UTF-8'}&configure=ilmsizechart"><i class="process-icon-back"></i> Back to List</a>
...
<div class="panel">
<h3><i class="icon-list-ul"></i> {l s='Configure Chart' mod='ilmsizechart'}</h3>
<table width="100%">
    <tr>
        <td>
