...
function product($id, $pref_s, $pref_e, $html_tags_status, $extra_feed_row, $one_branch, $only_enabled, $split_feed_limit, $part, $categories, $cat_list, $multistoreString)
{
    ...
    foreach ($xml_d as $xdd) {
        ...
        foreach ($xdd as $id => $val) {
            ...
            if ($id == 'product_out_of_stock') {
                $val = $val == 2 ? 'in stock' : 'out of stock';
            }
            if ($id == 'product_price') {
                $val = round($val, 2);
            }
        ...
        $xml .= '<g:brand>Indira</g:brand>';
        $xml .= '<g:age_group>adult</g:age_group>';
        $xml .= '<g:google_product_category>Apparel &amp; Accessories &gt; Jewelry</g:google_product_category>';
        $xml .= '<g:material>Argint</g:material>';
