...
/******************Change this*******************/
$api_url = 'https://updates.kairaweb.com/api/nikkon/';
/************************************************/
...
add_filter( 'themes_api', 'nikkon_api_call', 10, 3 );

function nikkon_api_call( $def, $action, $args ) {
	...
	if ( is_wp_error( $request ) ) {
		$res = new WP_Error( 'themes_api_failed', __( 'An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'nikkon' ), $request->get_error_message() );
	} else {
		$res = unserialize( $request['body'] );
		
		if ($res === false)
			$res = new WP_Error( 'themes_api_failed', __( 'An unknown error occurred', 'nikkon' ), $request['body'] );
	}
